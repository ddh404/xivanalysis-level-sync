...
export abstract class Utilities extends Analyser {
	...
	/**
	 * Optional level gate map, keyed by action ID.
	 * Actions not listed here are not filtered.
	 */
	protected actionLevelGates: Partial<Record<number, {minLevel?: number, maxLevel?: number}>> = {}
	...
	protected isActionEligible(action: Action): boolean {
		const level = this.actors.current.level
		if (level == null) { return true }
		const gate = this.actionLevelGates[action.id]
		if (gate == null) { return true }
		if (gate.minLevel != null && level < gate.minLevel) { return false }
		if (gate.maxLevel != null && level > gate.maxLevel) { return false }
		return true
	}
	...
	override output() {
		const eligibleActions = this.trackedActions.filter(a => this.isActionEligible(a))
	...
	public usageAvailableAtTimestamp(action: Action, timestamp: number = this.parser.pull.timestamp): boolean {
		if (!this.isActionEligible(action)) { return false }
