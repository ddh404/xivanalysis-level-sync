...
export interface CooldownGroup {
	...
	cooldowns: Action[],
	/**
	 * Minimum player level required to track this group.
	 * If the player's level is below this value, the group will be ignored.
	 */
	minLevel?: number,
	/**
	 * Maximum player level for which this group applies.
	 * Use this when a skill is replaced or upgraded at a higher level.
	 */
	maxLevel?: number,
	/**
...
export abstract class CooldownDowntime extends Analyser {
	...
	protected isGroupEligible(group: CooldownGroup): boolean {
		const level = this.actors.get(this.parser.actor).level
		if (level == null) { return true }
		if (group.minLevel != null && level < group.minLevel) { return false }
		if (group.maxLevel != null && level > group.maxLevel) { return false }
		return true
	}

	private getTrackedGroup(abilityId: number): CooldownGroup | undefined {
		return this.allCooldowns.find(group => {
			if (!this.isGroupEligible(group)) { return false }
			return group.cooldowns.find(action => action.id === abilityId) !== undefined
		})
	}
	...
	protected onComplete() {
		...
		for (const cdGroup of this.trackedCds) {
			if (!this.isGroupEligible(cdGroup)) { continue }
