...
export interface ActionRowConfig {
	...
	* */
	...
	/** Player must be at least this level for the row to be displayed. */
	minLevel?: number
	/** Player must be at most this level for the row to be displayed. */
	maxLevel?: number
}
...
export class ActionTimeline extends Analyser {
	...
	override initialise() {
		...
		for (const config of (this.constructor as typeof ActionTimeline).rows) {
			...
			if (!this.isRowEligible(resolvedConfig)) { continue }
	...
	private isRowEligible(config: InternalRowConfig): boolean {
		const level = this.actors.current.level
		if (level == null) { return true }
		if (config.minLevel != null && level < config.minLevel) { return false }
		if (config.maxLevel != null && level > config.maxLevel) { return false }
		return true
	}
